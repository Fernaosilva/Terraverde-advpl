...
/*/
//--------------------------------------------------------------------
User Function UPDANEXO( cEmpAmb, cFilAmb )
Local   aSay      := {}
Local   aButton   := {}
Local   aMarcadas := {}
...
If lOk
	...
	If !Empty( aMarcadas )
		If lAuto .OR. MsgNoYes( "Confirma a atualização dos dicionários ?", cTitulo )
			...
			If lAuto
				If lOk
					MsgStop( "Atualização realizada.", "UPDANEXO" )
				Else
					MsgStop( "Atualização não realizada.", "UPDANEXO" )
				EndIf
				dbCloseAll()
			Else
				If lOk
...
aAdd( aSX2, { ;
	'ZG9'																	, ; //X2_CHAVE
	cPath																	, ; //X2_PATH
	'ZG9'+cEmpr																, ; //X2_ARQUIVO
	'Anexos Geeker'															, ; //X2_NOME
	'Anexos Geeker'															, ; //X2_NOMESPA
	'Anexos Geeker'															, ; //X2_NOMEENG
	'E'																		, ; //X2_MODO
	''																		, ; //X2_TTS
	''																		, ; //X2_ROTINA
	''																		, ; //X2_PYME
...
aAdd( aSX3, { ;
	...
	12																		, ; //X3_TAMANHO
	0																		, ; //X3_DECIMAL
	'Tipo Chave'															, ; //X3_TITULO
	'Tipo Chave'															, ; //X3_TITSPA
	'Tipo Chave'															, ; //X3_TITENG
	'Tipo Chave'															, ; //X3_DESCRIC
	'Tipo Chave'															, ; //X3_DESCSPA
	'Tipo Chave'															, ; //X3_DESCENG
	''																		, ; //X3_PICTURE
	''																		, ; //X3_VALID
	'x       x       x       x       x       x       x       x       x       x       x       x       x       x       x x', ; //X3_USADO
	''																		, ; //X3_RELACAO
...
aAdd( aSX3, { ;
	...
	'Diretorio'																, ; //X3_TITSPA
	'Diretorio'																, ; //X3_TITENG
	'Diretorio'																, ; //X3_DESCRIC
	'Diretorio'																, ; //X3_DESCSPA
	'Diretorio'																, ; //X3_DESCENG
	''																		, ; //X3_PICTURE
	''																		, ; //X3_VALID
	'x       x       x       x       x       x       x       x       x       x       x       x       x       x       x x', ; //X3_USADO
	''																		, ; //X3_RELACAO
...
aAdd( aSIX, { ;
	'ZG9'																	, ; //INDICE
	'2'																		, ; //ORDEM
	'ZG9_FILIAL+ZG9_TP+ZG9_CHAVE+ZG9_ID'									, ; //CHAVE
	'Tipo Chave+Chave+ID'													, ; //DESCRICAO
	'Tipo Chave+Chave+ID'													, ; //DESCSPA
	'Tipo Chave+Chave+ID'													, ; //DESCENG
	'U'																		, ; //PROPRI
	''																		, ; //F3
	''																		, ; //NICKNAME
	'S'																		} ) //SHOWPESQ
...
@ 112, 157  Button oButOk   Prompt "Processar"  Size 32, 12 Pixel Action (  RetSelecao( @aRet, aVetor ), IIf( Len( aRet ) > 0, oDlg:End(), MsgStop( "Ao menos um grupo deve ser selecionado", "UPDANEXO" ) ) ) ;
