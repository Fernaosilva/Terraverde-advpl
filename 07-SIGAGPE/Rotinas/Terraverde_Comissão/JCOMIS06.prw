...
IF !Empty(Alltrim(Dtos(MV_PAR06)))
	cCmdSql += "  AND  ( SE1.E1_BAIXA = ' '  OR  SE1.E1_BAIXA >= '"+Dtos(MV_PAR06)+"' ) "  // pARA rps
Else
...
If !Empty(Alltrim(MV_PAR05))
	cCmdSql += " AND SF2.F2_SERIE    = '"+Alltrim(MV_PAR05)+"'" // IN ('RPS','E')   " 
Endif
...
cCmdSql += " SELECT E1_FILIAL  AS FILPROC, E1_NUM, E1_PREFIXO, SUM(F2_VALBRUT) AS VALOR , F2_EMISSAO" 
cCmdSql += " FROM  " + RetSqlName("SE1") + " SE1 
cCmdSql += " INNER JOIN " + RetSqlName("SF2") + " SF2 ON " 
cCmdSql += "     SF2.D_E_L_E_T_ = ' '   " 
cCmdSql += " AND SF2.F2_DUPL   <> ' '   "
cCmdSql += " AND SF2.F2_DOC     >= '"+MV_PAR01+"'"
cCmdSql += " AND SF2.F2_DOC     <= '"+MV_PAR02+"'"
cCmdSql += " AND SF2.F2_FILIAL  >= '"+MV_PAR03+"'"
cCmdSql += " AND SF2.F2_FILIAL  <= '"+MV_PAR04+"'"
cCmdSql += " AND SF2.F2_EMISSAO <= '"+DtoS(CtoD('20/08/2024')) + "' "
cCmdSql += " AND SE1.E1_PREFIXO  = SF2.F2_SERIE  " 

If !Empty(Alltrim(MV_PAR05))
	cCmdSql += " AND SF2.F2_SERIE    = '"+Alltrim(MV_PAR05)+"'" 
Endif

cCmdSql += " AND SE1.E1_FILIAL  = SF2.F2_FILIAL    "
cCmdSql += " AND SE1.E1_NUM     = SF2.F2_DOC    "
cCmdSql += " AND ( SE1.E1_PREFIXO  = SF2.F2_SERIE OR SE1.E1_PREFIXO  = 'DUP' OR SE1.E1_PREFIXO  = 'NEG' )  "
cCmdSql += " WHERE " 

IF !Empty(Alltrim(Dtos(MV_PAR06)))
	cCmdSql += "  ( SE1.E1_BAIXA = ' '  OR  SE1.E1_BAIXA >= '"+Dtos(MV_PAR06)+"' ) "  // pARA rps
Else
	cCmdSql += "  ( SE1.E1_BAIXA = ' '  OR  SE1.E1_BAIXA >= '20230101' ) "  // pARA rps
Endif
