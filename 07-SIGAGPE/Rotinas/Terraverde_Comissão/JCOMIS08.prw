...
If nOrdem == 1		// Ordem: por Titulo
	...
	cOrder := "E3_VEND,E3_FILIAL,E3_PREFIXO,E3_NUM,E3_PARCELA"
Else										// Ordem: por Cliente
	dbSetOrder(3)
...
While !oReport:Cancel() .And. !&(cAlias)->(Eof())
	...
	lVend:= .T. 
	lFirst := .T. 
	While !Eof() .And. /*cFil*/ /*xFilial("SE3")*/ /*== (cAlias)->E3_FILIAL .And.*/ (cAlias)->E3_VEND == cVend
		...
		dbSelectArea("SE3")
		dbSetOrder(2)
		dbSeek(cFil/*xFilial("SE3")*/+cVend+&(cAlias)->(E3_PREFIXO)+&(cAlias)->(E3_NUM)+&(cAlias)->(E3_PARCELA)+&(cAlias)->(E3_SEQ))
						
		dbSelectArea("SE1")
		dbSetOrder(1)
		dbSeek(cFil/*xFilial()*/+&(cAlias)->(E3_PREFIXO)+&(cAlias)->(E3_NUM)+&(cAlias)->(E3_PARCELA)+&(cAlias)->(E3_TIPO))
	   ...
	   // Verifica filtro do usuario
		...
		If!Empty(cFilSA1) 
		   	SA1->(dbSetOrder(1))               
			If SA1->(dbSeek(cFil/*xFilial()*/+&(cAlias)->(E3_CODCLI)+&(cAlias)->(E3_LOJA)))
				If !( SA1->&cFilSa1)	
		  			dbSelectArea(cAlias)	 
				   	dbSkip()  
					Loop
		...
		If Eof()
			...
			If AllTrim((cAlias)->E3_TIPO) == "NCC"
				SF1->(dbSeek(cFil /*xFilial("SF1")*/+(cAlias)->E3_NUM+(cAlias)->E3_PREFIXO+(cAlias)->E3_CODCLI+(cAlias)->E3_LOJA,.t.))
			...
			Else
		   		dbSeek(cFil/*xFilial()*/+(cAlias)->E3_NUM+(cAlias)->E3_PREFIXO)
